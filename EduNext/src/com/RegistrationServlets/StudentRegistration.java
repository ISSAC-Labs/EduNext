...
("jdbc:mysql://localhost:3306/EduNext","root","admin");
	   ...
	   int i = st.executeUpdate("insert into stud_reg (email_id, pwd ,age , phone, gender ) values ('" + email_id + "','" 

+ pwd + "','" +age + "','" + phone + "','" + gender + "')");
	   
	   ResultSet rs;
	   
	   rs = st.executeQuery("select * from stud_reg where email_id='" + email_id + "'");
	   ...
	   while(rs.next())
	   {
	    ...
	    if(i>0)
	    {
	    	...
	    	sendMail("javamailsystem1@gmail.com",email_id,"Account activation link",
	    			"Please click on the following link to activate your account  --> http://localhost:8000/EduNext/stud_profile.jsp?key=",key);
	    }
